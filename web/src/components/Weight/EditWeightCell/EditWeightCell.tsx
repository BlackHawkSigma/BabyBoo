...
export const Success = ({ weight }: CellSuccessProps<EditWeightById>) => {
  const [updateWeight, { loading, error }] = useMutation(UPDATE_WEIGHT_MUTATION, {
    onCompleted: () => {
  ...
  return (
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">Edit Weight {weight.id}</h2>
      </header>
      <div className="rw-segment-main">
        <WeightForm weight={weight} onSave={onSave} error={error} loading={loading} />
      </div>
    </div>
  )
}
